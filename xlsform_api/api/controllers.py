...
def post():
    with TemporaryDirectory() as temp_dir_name:
        try:
            with open(os.path.join(temp_dir_name, "tmp.xml"), "w+") as xform, open(os.path.join(temp_dir_name, "tmp.xlsx"), "wb") as xlsform:
                ...
                if os.path.isfile(xform.name):
                    return response(status=GOOD_REQUEST_CODE, result=xform.read(), warnings=convert_status)
                else:
